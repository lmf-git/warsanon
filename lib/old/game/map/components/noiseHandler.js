...
export default class NoiseHandler {
    ...
    static initialise() {
        MapData.heightNoise = new Noise(MapData.world.config.seed - 10);
        MapData.moistureNoise = new Noise(MapData.world.config.seed + 10);
    }

    static calcTileMoisture(x, y) {
        const noise = MapData.moistureNoise.perlin2(x / 10, y / 10);
        const noiseInt = Math.abs(noise);
        return noiseInt;
    }

    static calcTileHeight(x, y) {
        const noise = MapData.heightNoise.perlin2(x / 10, y / 10);
        const noiseInt = Math.abs(noise);
        return noiseInt;
    }
